...
function(tree,rowIx,count,ruleSet,rule,levelX,length,max_length,liSet,ruleLi=list())
{
  ...
  if( tree[rowIx,"status"] == -1 | length == max_length ){
    ...
    ruleSet[[count]] = rule
    liSet[[count]] = ruleLi
    return(list(ruleSet = ruleSet, count=count, prediction=tree[rowIx, "prediction"], liSet=liSet))
  }
...
#   rValue <- paste("X[,",xIx, "]%in% '",rValue,"'",sep="")
   ...
   ruleSet = thisList$ruleSet; count = thisList$count; prediction = thisList$prediction; liSet = thisList$liSet;

   
   thisList = treeVisit(tree, tree[rowIx,"right daughter"],count,ruleSet,ruleright,levelX,length+1,max_length,liSet,ruleLiRight)
   ruleSet = thisList$ruleSet; count = thisList$count; prediction = thisList$prediction; liSet = thisList$liSet;

   return(list(ruleSet = ruleSet, count=count, prediction=prediction, liSet=liSet))
}
